...
class EncoderFLCW(nn.Module):
    def __init__(self,bev_h=50,bev_w=50,num_feature_levels = 4,num_cams=4,embed_dims=256):
        ...
        self.final_reference_points_cam = nn.Parameter(torch.from_numpy(np.load('./F2BEV/pre_computation/unity_data/reference_points_cam.npy')).float(),requires_grad=False)
        self.final_bev_mask = nn.Parameter(torch.from_numpy(np.load('./F2BEV/pre_computation/unity_data/bev_mask.npy')).float(),requires_grad = False)
    ...
    def get_bev_features(self,mlvl_feats,bev_queries,bev_pos,can_buses,prev_bev=None):
        ...
        ## bev queries && pos torch.Size([2500, 2, 256])
        ...
        can_bus = bev_queries.new_tensor(
            np.array([each.numpy() for each in can_buses]))  # [:, :]
        # can_bus = bev_queries.new_tensor(
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            # print(feat.shape) 
            # torch.Size([4, 2, 21600, 256])
            # torch.Size([4, 2, 5440, 256])
            # torch.Size([4, 2, 1360, 256])
            # torch.Size([4, 2, 340, 256])
        ...
        # tensor([    0, 21600, 27040, 28400], device='cuda:0')
        feat_flatten = feat_flatten.permute(0, 2, 1, 3)  # (num_cam, H*W, bs, embed_dims)
        # torch.Size([4, 28740, 1, 256])
    ...
    def forward(self,mlvl_feats,can_buses,prev_bev=None):
        ...
        # hybrid_ref_2d torch.Size([4, 2500, 1, 2])
        ### todo add some more shift here; 2d reference business

        # bev queries torch.Size([2, 2500, 256])
        # feat_flatten # torch.Size([4, 28740, 2, 256])
