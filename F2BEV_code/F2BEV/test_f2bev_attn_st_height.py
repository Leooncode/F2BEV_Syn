...
from model_f2bev_attn_st_height import FisheyeBEVFormer
...
# if not os.path.exists('./predictions/f2bev_attn_st_height/'):
#    os.makedirs('./predictions/f2bev_attn_st_height/')
# if not os.path.exists('./predictions/f2bev_attn_st_height/bevfeatures'):
#    os.makedirs('./predictions/f2bev_attn_st_height/features')
# if not os.path.exists('./predictions/f2bev_attn_st_height/predfull/'):
#    os.makedirs('./predictions/f2bev_attn_st_height/predfull/')
# if not os.path.exists('./predictions/f2bev_attn_st_height/predfull/ce/'):
#    os.makedirs('./predictions/f2bev_attn_st_height/predfull/ce/')
...
if not os.path.exists('./predictions/'):
   ...
   os.makedirs('./predictions/f2bev_attn_st_height/bevfeatures')
...
target_transforms = torch.nn.Sequential(T.Resize((50,50)),T.Grayscale(num_output_channels=1))
...
checkpoint = torch.load('../f2bev_attn_st_height.pt')
        ...
        for batch_idx, (dataseq, targetseq) in enumerate(test_dataloader):
            ...
            for ctr in range(seq_len):
                ...
                if batch_idx == 0:
                    ...
                    prev_bev = torch.Tensor([np.load('./predictions/f2bev_attn_st_height/bevfeatures/'+image_list[batch_idx-1].split('.')[0]+'.npy')]).to(device)
                pred, _, for_prev_bev = model(front,left,rear,right, list(can_buses),prev_bev)
                print(batch_idx)
                for i,p in enumerate(for_prev_bev):
                    np.save('./predictions/f2bev_attn_st_height/bevfeatures/'+image_list[batch_idx].split('.')[0]+'.npy',p.detach().cpu().numpy())
                for i,p in enumerate(pred):
                    np.save('./predictions/f2bev_attn_st_height/predfull/ce/'+image_list[batch_idx].split('.')[0]+'.npy',p.detach().cpu().numpy())
                test_loss += loss_fn(pred, target).item()
                test_iou += jaccard_index(pred,target.to(pred.device),num_classes=3,average='none',  task="multiclass")
