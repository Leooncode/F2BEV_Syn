...
if not os.path.exists('./predictions/f2bev_attn_st_seg/'):
   os.makedirs('./predictions/f2bev_attn_st_seg/')
if not os.path.exists('./predictions/f2bev_attn_st_seg/bevfeatures'):
   os.makedirs('./predictions/f2bev_attn_st_seg/bevfeatures')
if not os.path.exists('./predictions/f2bev_attn_st_seg/predfull/'):
   os.makedirs('./predictions/f2bev_attn_st_seg/predfull/')
if not os.path.exists('./predictions/f2bev_attn_st_seg/predfull/ce/'):
   os.makedirs('./predictions/f2bev_attn_st_seg/predfull/ce/')

num_data_sequences = 2
...
checkpoint = torch.load('../f2bev_attn_st_seg (1).pt') 
        ...
        for batch_idx, (dataseq, targetseq) in enumerate(test_dataloader):
            ...
            for ctr in range(seq_len):
                ...
                if batch_idx == 0:
                    ...
                    prev_bev = torch.Tensor([np.load('./predictions/f2bev_attn_st_seg/bevfeatures/'+image_list[batch_idx-1].split('.')[0]+'.npy')]).to(device)
                pred, _, for_prev_bev = model(front,left,rear,right, list(can_buses),prev_bev)

                print(batch_idx)
                for i,p in enumerate(for_prev_bev):
                    np.save('./predictions/f2bev_attn_st_seg/bevfeatures/'+image_list[batch_idx].split('.')[0]+'.npy',p.detach().cpu().numpy())
                for i,p in enumerate(pred):
                    np.save('./predictions/f2bev_attn_st_seg/predfull/ce/'+image_list[batch_idx].split('.')[0]+'.npy',p.detach().cpu().numpy())
                test_loss += loss_fn(pred, target).item()
                test_iou += jaccard_index(pred,target.to(pred.device),num_classes=5,average='none',task="multiclass")
