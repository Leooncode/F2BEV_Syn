...
#     names = fnmatch.filter(os.listdir(bev_dir), '*BEV*.png')
#     files = []
#     for name in names:
...
# target_transforms = T.Compose([T.Grayscale(num_output_channels=1)])
...
vbev_dirs = ['./data/SynWood/semantic_annotations/BEV_images']
vfront_dirs = ['./data/SynWood/rgb_images/front_images']
vleft_dirs = ['./data/SynWood/rgb_images/left_images']
vrear_dirs = ['./data/SynWood/rgb_images/rear_images']
vright_dirs = ['./data/SynWood/rgb_images/right_images']
vconfig_dirs = ['./data/SynWood/vehicle_data/rgb_images']
vimage_lists = []
vdatalengths = []
...
def train(epoch,model,train_dataloader,seq_len, criterion,optimizer):
    ...
    for batch_idx, (dataseq, targetseq, gt_map) in enumerate(train_dataloader):
        ...
        for ctr in range(seq_len):
            ...
            optimizer.zero_grad()
            output, prev_bev_org = model(front,left,rear,right, list(can_buses),prev_bev)
    ...
    with torch.no_grad():
        for batch_idx, (dataseq, targetseq) in enumerate(test_dataloader):
            ...
            for ctr in range(seq_len):
                ...
                if ctr == 0:
                    prev_bev = None
                pred, prev_bev = model(front,left,rear,right, list(can_buses),prev_bev)
...
PATH = './Syn_f2bev_conv_st_seg.pt'
min_val_loss = np.inf

for epoch in range(n_epochs):
    train_loss = train(epoch,model,train_dataloader,seq_len,criterionFocal,optimizer)
    val_loss = val(epoch,val_dataloader,seq_len,model,criterionFocal)
